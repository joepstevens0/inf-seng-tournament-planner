...
router.get('/read', (req, res) => {
    (async () => {
        try {
            ...
            return res.status(200).send(lobbyDocs.docs.map((doc) => doc.data()));
        }
        catch (error) {
            console.error(error);
            return res.status(500).send(error);
...
router.get('/readbygame/:gameName', (req, res) => {
    (async () => {
        try {
...
router.post('/create', (req, res) => {
    (async () => {
        try {
            ...
            const correctToken = await authToken_1.authenticateToken(lobbyData.ownerId);
            if (!correctToken)
                return res.status(401).send('Unauthorized');
            // create a unique ID
            lobbyData.id = await userIdGeneration_1.generateUniqueID();
            // create new chat for lobby
            const chatid = await userIdGeneration_1.generateUniqueID();
...
router.put('/join', (req, res) => {
    (async () => {
        try {
            ...
            const correctToken = await authToken_1.authenticateToken(joinData.playerId);
            if (!correctToken)
                return res.status(401).send('Unauthorized');
            ...
            await db.runTransaction(async (transaction) => {
                ...
                await notificationHelper_1.handleNotification('A player joined your lobby!', 'A new player just joined your lobby, go check it out!', '/lobby/' + lobbyData.id, [lobbyData.ownerId]);
...
router.delete('/removeLobbyPlayer', async (req, res) => {
    try {
        ...
        // authenticate user
        const correctToken = await authToken_1.authenticateToken(ownerId);
        if (!correctToken)
            return res.status(401).send('Unauthorized');
        ...
        await notificationHelper_1.handleNotification('You have been removed from a lobby!', 'The leader of the lobby has removed you from his lobby, you can go and finc a new one!', '/teamfinder', [playerId]);
        return res.status(200).send();
    }
    catch (error) {
        console.error(error);
...
router.post('/toteam', (req, res) => {
    (async () => {
        try {
            ...
            // authenticate user
            const correctToken = await authToken_1.authenticateToken(playerId);
            if (!correctToken)
                return res.status(401).send('Unauthorized');
            ...
            if (lobbyData.ownerId != playerId)
                return res.status(401).send('Unauthorized');
            // check if lobby is full
            if (lobbyData.playerIds.length < lobbyData.maxPlayers)
                return res.status(401).send('Unauthorized');
            // create team id
            const teamId = await userIdGeneration_1.generateUniqueID();
            // get tournament
            ...
            if (tournamentData.teams.length >= tournamentData.amountTeams)
                return res.status(401).send('Unauthorized');
            // check if tournament has begun
            if (tournamentData.hasBegun)
                return res.status(401).send('Unauthorized');
            // start transaction
            await db.runTransaction(async (transaction) => {
            ...
            await notificationHelper_1.handleNotification("Team created", "The lobby '" + lobbyData.name + "' is now a team with name '" + teamName + "'", "/tournament/" + lobbyData.tournamentId, lobbyData.playerIds);
            return res.status(200).send();
        }
        catch (error) {
            console.error(error);
 ...
 */
async function deleteLobby(lobby, transaction) {
...
//# sourceMappingURL=lobbyRouter.js.map