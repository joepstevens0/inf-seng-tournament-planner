...
import { authenticateAdminToken, authenticateToken } from '../services/authToken';
	...
	(async () => {
		try {
			...
			await handleNotification(
				...
				'Welcome to this platform, enjoy your time on here!',
...
router.get('/readnick/:nickname', (req: Request, res: Response) => {
	(async () => {
		try {
			let foundDocs: Array<firestore.DocumentSnapshot> = [];
			await db
				.collection('users')
			...
			if (foundDocs.length == 0) return res.status(404).send('User Not Found');
			const userData = foundDocs[0].data() as any;
			userData.password = '';
			return res.status(200).send(JSON.stringify(userData));
		} catch (error) {
			console.error(error);
...
// BAN USER
router.put('/ban', async (req: Request, res: Response) => {
	try {
		const adminId = req.body.adminId;
		const userId = req.body.userId;

		const correctToken = await authenticateAdminToken(adminId);
		if (!correctToken) {
			return res.status(401).send({ message: 'Unauthorized' });
		}
...
// UNBAN USER
router.put('/unban', async (req: Request, res: Response) => {
	try {
		const adminId = req.body.adminId;
		const userId = req.body.userId;
...
router.put('/verify', async (req: Request, res: Response) => {
	try {
		...
		await handleNotification(
			...
			'You are now a verified user on this platform! You now belong to the top content creators!',
			'/profile/' + userRef.get('nickname'),
			[ userRef.get('id') ]
		);
		return res.status(200).send();
	} catch (error) {
...
// UNVERIFY USER
router.put('/unverify', async (req: Request, res: Response) => {
	try {
		const adminId = req.body.adminId;
		const userId = req.body.userId;
...
async function checkEmailExists(email: string): Promise<boolean> {
	...
	await db.collection('users').where('email', '==', email).get().then((querySnapshot: firestore.QuerySnapshot) => {
		querySnapshot.forEach((doc) => {
			userMatches.push(doc.data());
		});
	});
 ...
 */
async function getUserfromId(id: string): Promise<firestore.DocumentSnapshot> {
...
router.post('/readallchatusers', async (req: Request, res: Response) => {
	(async () => {
		try {
			let userData = req.body;
			if (userData.userId === '') {
...
async function addOtherChatUsers(
	user: firestore.DocumentSnapshot,
	result: firestore.DocumentSnapshot[]
): Promise<firestore.DocumentSnapshot[]> {
	let chatMatches: Array<firestore.DocumentSnapshot> = [];
	// Check if there's an existing user with this nickname
	await db
		.collection('chats')
		.where('usage', '==', 'p2p')
	...
	for (let i = 0; i < chatMatches.length; i++) {
		if (chatMatches[i].get('users')[0] == user.get('id') && !isDuplicate(chatMatches[i].get('users')[1], result)) {
			...
			const userData = tempUser.data() as any;
			userData.password = '';
...
function isDuplicate(id: string, list: firestore.DocumentSnapshot[]): Boolean {
...
async function getFriends(loggedUser: firestore.DocumentSnapshot): Promise<firestore.DocumentSnapshot[]> {
	let result = [];
	for (let i = 0; i < loggedUser.get('following').length; i++) {
		let tempUser = await getUserfromId(loggedUser.get('following')[i]);
		if (
			loggedUser.get('following').includes(tempUser.get('id')) &&
