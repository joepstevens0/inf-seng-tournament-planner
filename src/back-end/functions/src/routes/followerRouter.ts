...
router.put('/update', async (req: Request, res: Response) => {
    (async () => {
        try {
            ...
            const correctToken: boolean = await authenticateToken(firstUser[0].id); 
            if (!correctToken)
            ...
            if (firstUser.length > 0 && secondUser.length > 0) {
                if (firstUser[0].get('following').includes(secondUser[0].get('id'))
                    ...
                    await handleNotification(firstUser[0].get('nickname') + ' unfollowed', firstUser[0].get('nickname') + ' unfollowed you!','/profile/' + firstUser[0].get('nickname'), [secondUser[0].get('id')])
                    return res.status(200).send('ok');
                }
                else {
                    // follow
                    await followUser(firstUser[0].get('id'), secondUser[0].get('id'))
                    await handleNotification(firstUser[0].get('nickname') + ' followed', firstUser[0].get('nickname') + ' followed you!','/profile/' + firstUser[0].get('nickname'), [secondUser[0].get('id')])
                    return res.status(200).send('ok');
                }
            }
            return res.status(500).send();
...
router.post('/readfriend', async (req: Request, res: Response) => {
    (async () => {
        try {
            let userData = req.body;
            if (userData.firstName === '' || userData.secondName === '') {
...
router.post('/readallfriends', async (req: Request, res: Response) => {
    (async () => {
        try {
            let userData = req.body;
            if (userData.userId === '') {
            ...
            for (let i = 0; i < firstUser[0].get('following').length; i++) {
                ...
                if (firstUser[0].get('following').includes(tempUser[0].get('id'))
                    ...
                    const userData = tempUser[0].data() as any;
                    userData.password = "";
...
router.post('/readfollowers', async (req: Request, res: Response) => {
    (async () => {
        try {
            let userData = req.body;
            if (userData.userId === '') {
...
router.post('/readfollowing', async (req: Request, res: Response) => {
    (async () => {
        try {
            let userData = req.body;
            if (userData.userId === '') {
...
async function checkUserVerified(userID: string) {
    let user: Array<firestore.DocumentSnapshot> = [];
    await db
        .collection('users')
        .where('id', '==', userID)
        .get()
 ...
 */
async function getUser(name: string): Promise<firestore.DocumentSnapshot[]> {
    let user: Array<firestore.DocumentSnapshot> = [];
    await db
        .collection('users')
        .where('nickname', '==', name)
...
async function unfollowUser(user1Id: string, user2Id: string) {
    await db
        .collection('users')
        .doc(user1Id)
        .update({
            following: firestore.FieldValue.arrayRemove(user2Id)
        })
    await db
        .collection('users')
        .doc(user2Id)
        .update({
            followers: firestore.FieldValue.arrayRemove(user1Id)
        })
}

//user1 follows user2
async function followUser(user1Id: string, user2Id: string) {
    await db
        .collection('users')
        .doc(user1Id)
        .update({
            following: firestore.FieldValue.arrayUnion(user2Id)
        })
    await db
        .collection('users')
        .doc(user2Id)
        .update({
            followers: firestore.FieldValue.arrayUnion(user1Id)
        })
