...
router.get('/p2pid/:id1/:id2', async (req: Request, res: Response) => {
	try {
		// authenticate user
		const correctToken: boolean = await authenticateToken(req.params.id1);
		if (!correctToken) return res.status(401).send('Unauthorized');

		// get chat from id1
		const chat1Ref = db.collection('chats').where("usage", "==", "p2p")
			.where(
				"users", "array-contains", req.params.id1);
		const chat1Docs = await chat1Ref.get();

		// get chats from id2
		const chat2Ref = db.collection('chats').where("usage", "==", "p2p")
			.where(
				"users", "array-contains", req.params.id2);
...
router.post('/create/:id', (req: Request, res: Response) => {
	(async () => {
		try {
			...
			// authenticate user
			const correctToken: boolean = await authenticateToken(messageData.senderId);
			if (!correctToken) return res.status(401).send('Unauthorized');

			// test if user in chat
			const chatDoc = await db.collection('chats').doc(req.params.id).get();
			const chatData = chatDoc.data();
			if (chatData !== undefined && chatData.users !== undefined && (chatData.users as string[]).indexOf(messageData.senderId) == -1)
...
// GET CHAT MESSAGES
router.get('/read/:id/:userId', async (req: Request, res: Response) => {
	...
	try {
		...
		const chatDoc = await db.collection('chats').doc(req.params.id).get();
		const chatData = chatDoc.data();
		if (chatData !== undefined && chatData.users !== undefined && (chatData.users as string[]).indexOf(userId) == -1)
			return res.status(401).send('Unauthorized');
		...
		return res.status(200).send(messageDocs.docs.map((doc: any) => doc.data()));
	} catch (error) {
		console.error(error);
...
// SUBSCRIBE TO A CHAT
router.get('/subscribe/:id/:userId', async (req: Request, res: Response) => {
 ...
 */
function sendMessageToClient(res: Response, data: any) {
 ...
 */
async function subscribeChat(res: Response, id: any) {
	...
	const unsubscribe = await messageCollection.onSnapshot(
		(querysnapshot: any) => {
			const newMessages = [] as any[];
			querysnapshot.docChanges().forEach((element: any) => {
				if (element.type == 'added') newMessages.push(element.doc.data());
			});

			// send messages to client
			if (newMessages.length > 0) sendMessageToClient(res, newMessages);
		},
		function (error: any) {
