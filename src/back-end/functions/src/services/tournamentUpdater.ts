...
export function selectWinner(
  ...
  switch (format) {
    case ScheduleFormat.SingleElim:
      return selectWinnerSingleElim(
        currentSchedule,
        currentStage,
        winnerIds
      );
    case ScheduleFormat.RoundRobin:
      return selectWinnerRoundRobin(
        currentSchedule,
        currentStage,
        scoreMap,
        winnerIds,
 ...
 */
function selectWinnerSingleElim(
  currentSchedule: Array<Array<any>>,
  currentStage: number,
  winnerIds: Array<string>
) {
  ...
  winnerIds.forEach((winner) => {
    ...
    if (subIndex == 0) {
      currentSchedule[currentStage + 1][
        Math.floor(winnerIndex / 2)
      ].team1Id = winner;
 ...
 */
function selectWinnerRoundRobin(
  currentSchedule: Array<Array<any>>,
  currentStage: number,
  scoreMap: IDictionary<number>,
  winnerIds: Array<string>,
...
function findWinnerIndex(
  ...
  for (let i = 0; i < currentSchedule[currentStage].length; ++i) {
    if (
      currentSchedule[currentStage][i].team1Id == winnerId ||
