 ...
 */
export function schedule(format: ScheduleFormat, teams: Array<string>) {
	let schedule: Array<Array<any>> = [];
	// Check if the length of our array is a power of 2
	if (!Number.isInteger(Math.log2(teams.length) || teams.length == 1)) return [];
	else {
		switch (format) {
 ...
 * @returns `Array<Array<Match>>` schedule
 */
function singleElimSchedule(teams: Array<string>) {
 ...
 * @returns `Array<Array<Match>` schedule
 */
function roundRobinSchedule(teams: Array<string>) {
